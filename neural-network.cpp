...
//
// Much of the following can be moved to a header, but to be able to operate in 
//   Compiler Explorer (single file compiler), stick to single source file
//

/*
//
// Simple, Fully connected Neural Net
//   with the hopes of evolving into a CNN (convolution NEural Network)
//
// Compiled with ARM gcc 8.2 (with -Os -std=c++17 -mtune=cortex-a72 or use -O3 which may give faster but more assembly)
//   hopefully this will allow it to run on the RPi 4
//
// or use X86-64 gcc 9.1 (with -Os -std=c++17/2a -Wall -Wextra)
//
// Implemented with what is known as Modern C++ (11, 14, 17 & 20 hopefully soon)
//   and designed to be cache friendly (as small and fast as possible)
*/
...
constexpr std::array topology{2u,3u,1u}; // layers/nodes : template argument deduction done in C++17 : number of weights = 2*3+3*1=9; any bias weights are accounted for in feedForward
...
/*
    Need generation of random numbers for Node weights at compile time (using constexpr)
    1. Generate a seed based on time
    2. Struct that houses the plumbing that generates a stream of random numbers
    3. Function that calls methods on PCG Struct and captures a random number
*/
...
private:
   constexpr std::uint32_t pcg32_random_r()
   {
      ...
      rng.state = oldstate * 6364136223846793005ULL + (rng.inc|1); // Magic number prefered by PCG
...
/*
    End of code block that can be used to generate random numbers at compile time
*/


//
// Collect neuron weights
//
template<typename T>
constexpr auto getRandomWeights()
{
   std::array<T, numWeights> weights{};  // numWeights is a const
...
//
// Arrays of values used throughout the feed forward and back propagation process
//   std::array can be cache friendly, quickly moved, and contribute to RVO
//
// Haven't decided if these will be moved into the Neural Network class,
//   it's possible as containers can be designed low cost
//
std::array<dataType, numWeights> targetValues {}; // the groundtruth for backPropagation
std::array<dataType, numWeights> nodeOutputValues {}; // running outputs of the nodes
std::array<dataType, numWeights> nodeGradients {};

//
// A weights matrix is what really describes the layesr/node makeup f the Neural Net
//   What each node does can be described in the feedForward operation
//
constexpr std::array weightsMatrix(getRandomWeights<dataType>());

//
// Network made up of layers, which are inturn, are made up of neurons
//
class NeuralNetwork
{
public:
